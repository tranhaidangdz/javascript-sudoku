...
// Hàm trợ giúp delay (chờ theo ms)
...
async function aiSolve() {
    ...
    async function solve() {
        for (let row = 0; row < CONSTANT.GRID_SIZE; row++) {
            for (let col = 0; col < CONSTANT.GRID_SIZE; col++) {
                if (su_answer[row][col] === CONSTANT.UNASSIGNED) {
                    for (let num = 1; num <= 9; num++) {
                        if (isSafe(su_answer, row, col, num)) {
                            // Ghi số vào mảng đáp án
                            su_answer[row][col] = num;
                            // Cập nhật giao diện: tìm ô tương ứng
                            let cellIndex = row * CONSTANT.GRID_SIZE + col;
                            cells[cellIndex].innerHTML = num;
                            cells[cellIndex].classList.add("selected");
                            await delay(200); // chờ 200ms để xem chuyển động
                            cells[cellIndex].classList.remove("selected");

                            if (await solve()) {
                                return true;
                            } else {
                                // Quay lui: xóa số vừa điền
                    ...
                    return false; // Không tìm được số phù hợp => quay lui
                }
            }
        }
